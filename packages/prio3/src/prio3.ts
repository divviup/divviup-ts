...
export class Prio3<Measurement> implements Prio3Vdaf<Measurement> {
  ...
  async initialPrepareState(
    ...
    if (flp.jointRandLen > 0 && share.blind && share.hint) {
      ...
      shareJointRandSeed = Buffer.from(
        await prg.deriveSeed(
          share.blind,
          Buffer.from([aggregatorId, ...encoded])
        )
      );
      jointRandSeed = Buffer.from(xor(share.hint, shareJointRandSeed));
  ...
  private async buildLeaderShare(
    ...
    const hint = Buffer.from(
      await prg.deriveSeed(blind, Buffer.from([0, ...encoded]))
    );
  ...
  private buildHelperShares(): Promise<Share[]> {
    ...
    return Promise.all(
      arr(this.shares - 1, async (j) => {
        ...
        const encoded = field.encode(inputShare);
        const hint = Buffer.from(
          await prg.deriveSeed(blind, Buffer.from([j + 1, ...encoded]))
        );
